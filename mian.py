...
if __name__ == '__main__':
    ...
    parser.add_option('-l', '--NEED_LOAD_FROM_DB', type=int, help="NEED_LOAD_FROM_DB", default=0)

    parser.add_option('-t', '--PATH_TO_TARGETS_EXCEL', type=str,
                      help="PATH_TO_TARGETS_EXCEL", default='data_/targets_excel/*.xls')
    ...
    parser.add_option('-i', '--PROJ_ID', type=int, help="PROJ_ID", default=46433)
    ...
    if NEED_LOAD_FROM_DB:
        ...
        if CONNECT:
            ...
            print_i(f"SUCCESS SAVE UNLOADING TO {PATH_EXCEL_PROJECTS+list(df['project_name'].unique())[0]+'.xlsx'}")
        # endregion
        ...
        if RELOAD_DOPS:
            ...
            assert len([name for name in os.listdir(DOP_DATA_PATH) if os.path.isfile(os.path.join(DOP_DATA_PATH, name))]
                       ) > 0, print_e("Закинь Norms.xlsx и stages в папку для доп материала")
            df_norm = pd.read_excel(DOP_DATA_PATH + 'Norms.xlsx')  # noqa
        ...
        if CONVERT:
            print_i(f'CONVERT XLSX DATA TO NPY')
            assert len([name for name in os.listdir(DOP_DATA_PATH) if os.path.isfile(os.path.join(DOP_DATA_PATH, name))]) > 1, print_e("Закинь данные в папку для доп материала")
            ...
            print_i('SUCCESS CONVERT TO NPY')
        # endregion
    # endregion

    # region NEED CREATE TARGET
    if TARGET:
        print_i('CREATE TARGET')
        assert len([name for name in os.listdir(DOP_DATA_PATH) if os.path.isfile(os.path.join(DOP_DATA_PATH, name))]) > 0, print_e("Указанный путь не содержит файлов")
        a = pd.DataFrame()
        year = 0
