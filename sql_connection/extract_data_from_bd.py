...
WITH table1 AS (
	...
	r.id_activity, dt, id_mat_res ,r.id_resource ,r.{FO} ,res.resource_name, res.resource_type
	...
	WHERE id_project = {proj_id} AND resource_type ='RT_Equip' and res.resource_name !='Стоимость'),
mat_table AS (
	SELECT 	concat(r.id_activity,'_',CAST(CAST(r.dt as float) as int),'_',r.id_resource) AS "key_connect",
	r.id_resource , r.dt, r.{FO}, res.resource_name
	FROM publication.RESASSIGNMENTSPRED r
	LEFT JOIN publication.RESOURCE res ON r.id_resource = res.id_resource
	WHERE id_project = {proj_id} AND resource_type ='RT_Mat'  and res.resource_name !='Стоимость')
	,
contractor AS(
SELECT   concat(r.id_activity,'_',CAST(CAST(r.dt as float) as int),'_',id_mat_res) AS "key_connect",
concat(r.id_activity,'_',CAST(CAST(r.dt as float) as int),'_',id_mat_res,'_',res.id_resource) AS "key_connect2",
...
n.PO_id,n.PO_coef, a.id_activity,
	...
	t1.resource_name, t1.id_resource, t1.{FO},
	mt.resource_name AS "mat_res_name", mt.id_resource AS "mat_id_res", mt.{FO} AS "mat_res_qt",
...
# 16  WHERE id_project = {proj_id} AND resource_type ='RT_Equip' and r.act_reg_qty IS NOT NULL and res.resource_name !='Стоимость'),
