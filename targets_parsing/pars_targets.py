        ...
        if not pd.isna(i):
            ...
            if (re.findall(r'итого', i.lower())) or (ind == df.shape[0]):
                ...
                assert round(sum(dict_res_name.values()), 0) == round(df.iloc[last_res_type_index, fact],
                       0), f'sum of machines = {sum(dict_res_name.values())} ' \
                           f'should be the same in Excel = {df.iloc[last_res_type_index, fact]}, ' \
                           f'last_resource_index ={last_res_type_index},current_index={ind}'
            ...
            if re.findall(r'ооо', i.lower()):
                ...
                if res_type_opened:  # если до этого 'res_type' был открыт
                    # тут записывается [ООО: 'ххх' , stage: 'xxx', res_type: 'xxx', res_name: 'xxx', 'hours': 'xxx']
                    # и аппендит в data
                    for k, v in dict_res_name.items():
                        data.extend(
            ...
            if stage_opened & pd.isna(df.iloc[ind, 3]) & (not re.findall(r'\(', i.lower())):
                ...
                if res_type_opened:  # если до этого 'res_type' был открыт
                    ...
                    # проверка на сумму машиночасов из словаря и из Экселя
                    if pd.isna(df.iloc[last_res_type_index, fact]):
                        print(
                            f' Missed data in file (sum of resources): row = {last_res_type_index}, column= {fact}: {df.iloc[last_res_type_index, fact]}')
                    else:
                        assert round(sum(dict_res_name.values()), 0) == round(df.iloc[last_res_type_index, fact],
                                                                              0), f'sum of machines = {sum(dict_res_name.values())} should be the same in Excel = {df.iloc[last_res_type_index, fact]}, {last_res_type_index},{ind}'
            ...
            if stage_opened & res_type_opened & (
                 (not pd.isna(df.iloc[ind, fact])) or (len(re.findall(r'\(', i.lower())) > 0) or (len(i.split()) > 1)):
                if pd.isna(df.iloc[ind, fact]):
                    continue  # На случай если данных по машине нет
        ...
        if not pd.isna(i):
            ...
            if re.findall(r'ооо*|есин|стройтранс*|газстройпро*', i.lower()):
                ...
                if (not pd.isna(df.iloc[ind + 1, 3]) and (ind > 10) and (not pd.isna(df.iloc[
                                                                                         ind - 1, 3]))):  # еще
                    # одно условие  если первое слишком слабое or ( not pd.isna(df.iloc[ind+1,8])
            ...
            if re.findall(r'этап', i.lower()):
                ...
                dict_stage = {'stage': i}
                res_type_opened = False  # это нужно закрывать на случай, если после этапа будет res_type и в
                # data занесется еще не заполненный res_type
                stage_opened = True
                continue
            ...
            if stage_opened & (pd.isna(df.iloc[ind, 3])) & (not re.findall(r'\(', i.lower())):
                ...
                if (pd.isna(df.iloc[ind, 8])) & (pd.isna(df.iloc[ind, fact])):
                    print_d('в строке нет ни часов ни вида техники. Значение этого ряда не будет учитываться', i)
                    continue
            ...
            if stage_opened & (res_type_opened) & (
                ...
                if pd.isna(df.iloc[ind, fact]) or type(df.iloc[ind, fact]) == str:
                    continue  # На случай если данных по машине нет
